...
function composeMessage(address: string, nonceExplorer: string, nonceRewards: string) {
  ...
  return [
    ...
    address,
    '',
    `Sign in to ${ config.chain.name || '' } Explorer & Rewards program. Merits nonce: ${ nonceRewards }.`.trim(),
    '',
    `URI: ${ urlObj.origin }`,
    'Version: 1',
  ...
  const getSiweMessage = React.useCallback(async(address: string) => {
    try {
      ...
      const nonceConfig = await apiFetch(
        'rewards:nonce',
        { queryParams: { explorer_login_address: address, explorer_login_chain_id: config.chain.id } },
