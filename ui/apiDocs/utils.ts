...
export const REST_API_SECTIONS = [
  config.apis.general && {
    id: 'core-api',
    title: `${ config.chain.name || 'Explorer' } Core API`,
    swagger: {
      // default swagger URL, will be replaced with an URL constructed from the backend version and the openapi spec folder name
      url: 'https://raw.githubusercontent.com/explorer/explorer-api-v2-swagger/main/swagger.yaml',
      requestInterceptor: (req: SwaggerRequest) => {
        ...
        if (!req.loadSpec) {
          const newUrl = (() => {
            try {
              const DEFAULT_SERVER = 'lux.network/poa/core';
