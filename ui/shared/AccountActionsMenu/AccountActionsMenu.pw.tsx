...
test.describe('with multiple items', () => {
  ...
  test('base view with styles', async({ render, page }) => {
    const component = await render(<AccountActionsMenu className="m-2 outline outline-1 outline-border"/>, { hooksConfig });
    await component.getByRole('button').click();
  ...
  test('loading with styles', async({ render }) => {
    const component = await render(<AccountActionsMenu isLoading className="m-2 outline outline-1 outline-border"/>, { hooksConfig });
