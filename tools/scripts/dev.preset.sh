...
if [[ "$preset_name" == "optimism_superchain" ]]; then
  dotenv \
    -e $config_file \
    -- bash -c 'cd deploy/tools/multichain-config-generator && pnpm install --silent && pnpm build && pnpm generate' || exit 1
fi

# generate essential dapps chains config if marketplace essential dapps enabled
dotenv \
  -e $config_file \
  -- bash -c 'cd deploy/tools/essential-dapps-chains-config-generator && pnpm install --silent && pnpm build && pnpm generate' || exit 1
